	...
	function outputfile($date) {
		...
		foreach($q->result() as $row) {
			if($row->legal_doctype_id != $curDocType) {
				...
				if($htmlStr != "")
					$htmlStr .= "<BR>-----------------------------------<BR>";
				$htmlStr .= "<BR><BR><H3>" . $doc->name . "</H3><BR><BR>";
				$noLine = true;
			}
			...
			//$pattern = "/<p[^>]*><\\/p[^>]*>/";
			//$htmlStr .= preg_replace($pattern, "", $row->adtext);
...
//
		//$htmlStr = 'Hello!'
		$htmlStr = str_replace("<br>",'',$htmlStr);
		$htmlStr = str_replace("<p>\xc2\xa0</p>",'',$htmlStr);
		$htmlStr = str_replace("<p>\xc2\xa0<br /></p>",'',$htmlStr);
		$htmlStr = str_replace("<p><STRONG> </STRONG></p>",'',$htmlStr);
		

		$this->load->helper("/rtfgen/rtf_helper.php");
		generateRTF($htmlStr, "Test Doc", "Output Fort ".$date);
	}
	...
	function generateproof($jobNumber) {
		...
		$adText = str_replace(array('<STRONG>', '<EM>', '<U>', '<strong>', '<em>', '<u>', '</STRONG>', '</EM>', '</U>', '</strong>', '</em>', '</u>', '<p>', '</p>' , '<P>', '</P>'), '', $adText);;
