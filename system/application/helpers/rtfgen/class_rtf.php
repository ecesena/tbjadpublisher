...
class rtf {
	...
	// Ouput the RTF file
	function getDocument($filename="rtf") {
		$this->buffer .= "{";
		// Header
...
function specialCharacters($text) { $utf8_patterns = array( "[\xC2-\xDF][\x80-\xBF]", "[\xE0-\xEF][\x80-\xBF]{2}", "[\xF0-\xF4][\x80-\xBF]{3}", ); $new_str = $text; foreach($utf8_patterns as $pattern) { $text = preg_replace("/($pattern)/e",  "'\u'.hexdec(bin2hex(mb_convert_encoding('$1', 'UTF-16', 'UTF-8'))).'?'",  $text); } return $text; }

	
	// Convert special characters to ASCII
	function escapeCharacter($character) {
		$escaped = "";
		$ordChr = ord($character);
		
		if(($ordChr >= 0x00 && $ordChr < 0x20) || ($ordChr >= 0x80 && $ordChr < 0xFF))
			$escaped = "\\'".dechex($ordChr);
		
		if (($ordChr >= 0x20 && $ordChr < 0x80) || $ordChr == 0x09 || $ordChr == 0x0A)
			$escaped = $character;

		if($ordChr == 0x5C || $ordChr == 0x7B || $ordChr == 0x7D)
			$escaped = "\\".$character;
	...
	function parseDocument() {
		...
		//Run three times to take care of nested tags
		for($i=0; $i<3; $i++) {
		...
		$doc_buffer = preg_replace("/<H1>(.*?)<\/H1>/mi", "\\f4\\fs48\\b \\1\\b0\\f{$this->font_face}\\fs{$this->font_size}\\par \\qj", $doc_buffer);
		$doc_buffer = preg_replace("/<H2>(.*?)<\/H2>/mi", "\\f4\\fs36\\b \\1\\b0\\f{$this->font_face}\\fs{$this->font_size}\\par \\qj", $doc_buffer);
		$doc_buffer = preg_replace("/<H3>(.*?)<\/H3>/mi", "\\f4\\fs27\\b \\1\\b0\\f{$this->font_face}\\fs{$this->font_size}\\par \\qj", $doc_buffer);
		
		
		$doc_buffer = preg_replace("/<HR(.*?)>/i", "\\brdrb\\brdrs\\brdrw30\\brsp20 \\pard\\par \\qj", $doc_buffer);
		$doc_buffer = str_replace("<BR>", "\\par ", $doc_buffer);
		$doc_buffer = str_replace("<br>", "\\par ", $doc_buffer);
		$doc_buffer = str_replace("<TAB>", "\\tab ", $doc_buffer);
		$doc_buffer = str_replace("\'c2\'a0", " ", $doc_buffer);
		$doc_buffer = utf8_encode($doc_buffer);
		$doc_buffer = str_replace(array("&amp;", "&nbsp;", "&lt;", "&gt;"), array("&", " ", "<", ">"), $doc_buffer);
